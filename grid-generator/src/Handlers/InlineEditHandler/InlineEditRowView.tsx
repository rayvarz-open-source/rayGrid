...
export default class InlineEditRowView extends React.Component<Props, any> {
    ...
    const mapArr = this.props.columns
      ...
      .map((columnDef, index) => {
        ...
        if (!columnDef.field || !allowEditing) {
          ...
          return (
            <TableCell key={columnDef.tableData.id} align={['numeric'].includes(columnDef.type) ? 'right' : 'left'}>
              {/* <EditComponent
                key={columnDef.tableData.id}
                columnDef={cellProps}
                value={value}
              ...
              {this.makeEditComponent(
                ...
                (_value) => {
                  const data = { ...this.state.data };
                  const value = _value.target !== null ? _value.target.value : _value;
                  setByString(data, columnDef.field, value);
                  // data[columnDef.field] = value;
                  this.setState({ data });
                },
  ...
  makeEditComponent(
    ...
    _onRowDataChange: (value: any) => void
  ) {
    if ([FieldSchemaTypeName.String, FieldSchemaTypeName.String].includes(field.type.name)) {
      return <TextField value={value} key={key} onChange={(value) => onChange(value)} type={'text'} />;
    } else if ([FieldSchemaTypeName.Money, FieldSchemaTypeName.Int].includes(field.type.name)) {
      return <TextField value={value} key={key} onChange={(value) => onChange(value)} type={'number'} />;
    } else if (field.type.name === FieldSchemaTypeName.Bit) {
      return (
        <Checkbox
          checked={value == true}
