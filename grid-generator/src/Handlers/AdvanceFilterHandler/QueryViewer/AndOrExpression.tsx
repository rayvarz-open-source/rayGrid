...
const useAndOrStyles = makeStyles((_theme: Theme) =>
  createStyles({
    keyword: {
      fontWeight: 'bold',
      transition: '200ms',
...
export function AndOrExpression(props: AndOrExpressionProps) {
  ...
  const createKeyword = (value: string, variant = 'h6') => (
...
function calculateExpressionDepth(expression: ExpressionModel): number {
  ...
  const depth = expression.value.length;
  const childrenDepths = expression.value.map((v) => calculateExpressionDepth(v));
