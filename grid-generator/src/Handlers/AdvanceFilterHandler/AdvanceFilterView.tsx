...
import { FieldSchemaTypeName, FieldType } from '../../Models/Field';
...
const DEFAULT_EXPRESSION: ExpressionModel = {
  ...
  extras: {
    field: {
      ...
      order: 0,
      title: '',
      type: { name: FieldSchemaTypeName.String, source: null },
    },
    filters: [],
  },
};
...
function insertInnerPath(source: ExpressionModel, expression: ExpressionModel, path: number[]) {
  const parent = getExpressionByPath(source, path);
  parent.value.push(expression);
}

function deletePath(source: ExpressionModel, path: number[]) {
  const child = getExpressionByPath(source, path);
  const parent = getExpressionByPath(source, path.slice(0, path.length - 1));
  ...
  return (
    <DragDropContext onDragEnd={onDragEnd} onDragStart={onDragStart}>
      <AdvanceFilterContext.Provider
        value={{ isDragging, onDropped, onDelete, contentProps: props, rootExpression: queryExpression }}
      >
...
export function AdvanceFilterView(props: AdvanceFilterViewProps) {
  ...
  React.useEffect(() => {
    const openSub = props.open.subscribe((v) => setOpen(v));
    const contentPropsSub = props.contentProps.subscribe((v) => setContentProps(v));
    return () => {
      openSub.unsubscribe();
