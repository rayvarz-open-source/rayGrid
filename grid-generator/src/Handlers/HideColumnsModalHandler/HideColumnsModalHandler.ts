...
export function setupHideColumnModal(
  ...
  function createControllerForField(
    ...
    const subject = new BehaviorSubject<boolean | null>(
      !hiddenFieldsSnapshot.includes(field.fieldName)
    );

    // update schema if value changed
    subject.pipe(skip(1), first()).subscribe((isChecked) => {
      if (!isChecked)
        hiddenFields.next([...hiddenFields.value, field.fieldName]);
      else
        hiddenFields.next([
          ...hiddenFields.value.filter((v) => v !== field.fieldName),
        ]);
    });
  ...
  function createCheckboxElements(
    ...
    for (let index = 0; index < chunkedFields.length; index++) {
      ...
      const children: IElement[] = chunk.map((field) =>
      ...
      if (
        index === chunkedFields.length - 1 &&
        index > 0 &&
        chunk.length < chunkedFields[index - 1].length
      ) {
        const numberOfPlaceholderElements =
          chunkedFields[index - 1].length - chunk.length;
        for (let i = 0; i < numberOfPlaceholderElements; i++)
  ...
  return Container(
    combineLatest(schema, hiddenFields).pipe(
      map(([schemaSnapshot, hiddenFieldsSnapshot]) => [
        ...createCheckboxElements(schemaSnapshot, hiddenFieldsSnapshot),
      ])
    )
  );
}
...
export const hideColumnsModalHandler: Handler = (props, observables) => {
  const openHideColumnsModal = (localization: Localization) => {
    props.elements.hideColumnModal.childContainer.next(
      setupHideColumnModal(
        props.controllers.schemaController,
        props.controllers.hideColumnModalHiddenFieldsController
      )
    );
    props.elements.hideColumnModal.titleContainer.next(
      localization.columnVisibilityTitle
    );
  ...
  props.controllers.schemaController.subscribe((v) => {
    props.controllers.hideColumnModalHiddenFieldsController.next(
      v.fields
        .filter((f) => f.isVisible && !f.isVisibleDefault)
        .map((f) => f.fieldName)
    );
  });
