...
export const remoteDataSourceHandler: Handler = (props, observables) => {
  ...
  const dataSourceObservable = new BehaviorSubject<GridElementAttributes.Datasource<any>>(async (query) => {
    const filters = materialTableFilterToGridFilter(query.filters, props.controllers.schemaController.value);
    if (query.search) {
      filters.push({
        fieldName: 'ALL',
    ...
    const needPageSize = lastFilters == null || isFilterChanged(lastFilters, filters);
    lastFilters = filters;

    const requestFilters = [...filters, ...props.controllers.advanceFiltersController.value];
            ...
            tableData: {
              checked:
                props.controllers.selectionController.value.filter(
                  (selected) =>
                    selected[props.controllers.keyFieldName.value] == v[props.controllers.keyFieldName.value]
                ).length > 0,
            },
          };
        }
...
export function processResultValue(originalItems: any[], schema: Schema): any[] {
  ...
  const refrenceNames = schema.fields.map((field) => `@${field.fieldName}`);

  const hasRefrenceName = (value: string) => {
    for (const refrenceName of refrenceNames) {
  ...
  schema.fields.forEach((field) => {
    if (
      ...
      for (const row of items) {
        ...
        for (const refrenceName of refrenceNames) col = col.replace(refrenceName, row[refrenceName.substring(1)] + '');
